...
from apps.booking.api_views import (
    BookingCreateView, BookingStatusView, BookingCancelView,
    GroupJoinView, GroupJoinStatusView, ContactCreateView,
)
from apps.core.api_views import JobApplicationCreateView
from apps.reviews.api_views import TourReviewCreateView
...
api_urlpatterns = [
    ...
    path("careers/<slug:job_slug>/apply/", JobApplicationCreateView.as_view(), name="api-careers-apply"),

    # Write endpoints — bookings, group-join, contact, reviews
    path("contact/", ContactCreateView.as_view(), name="api-contact"),
    path("bookings/", BookingCreateView.as_view(), name="api-booking-create"),
    path("bookings/<str:booking_ref>/<str:secure_token>/", BookingStatusView.as_view(), name="api-booking-status"),
    path("bookings/<str:booking_ref>/<str:secure_token>/cancel/", BookingCancelView.as_view(), name="api-booking-cancel"),
    path("group-departures/<slug:slug>/join/", GroupJoinView.as_view(), name="api-group-join"),
    path("group-departures/join/<str:member_id>/<str:secure_token>/", GroupJoinStatusView.as_view(), name="api-group-join-status"),
    path("tours/<slug:slug>/reviews/", TourReviewCreateView.as_view(), name="api-tour-review-create"),
